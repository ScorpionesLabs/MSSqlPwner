...
DATABASES_LIST = "SELECT name FROM sys.databases WHERE database_id > 4"
...
CREATE_PROCEDURE = "CREATE PROCEDURE [{procedure_name}] {arg} AS EXTERNAL NAME [{asm_name}].[StoredProcedures].[{procedure_name}];"
IS_FUNCTION_EXISTS = "SELECT CASE WHEN OBJECT_ID('{function_name}') IS NOT NULL THEN 'True' ELSE 'False' END as status;"
CREATE_FUNCTION = "CREATE FUNCTION [{db_user}].{function_name}({arg}) RETURNS NVARCHAR(MAX) AS EXTERNAL NAME {asm_name}.[{namespace}.{class_name}].{function_name};"
...
FUNCTION_EXECUTION = "SELECT {db_user}.{function_name}({command});"
