...
class CustomThread(Thread):
    def __init__(
        self, group=None, target=None, name=None, args=(), kwargs: dict = None
    ):
        if kwargs is None:
            kwargs = {}
        self._kwargs = None
...
def generate_string(
    size=6, chars=string.ascii_uppercase + string.ascii_lowercase
) -> str:
    """
...
def count_quotes(my_str, index_to_find, quote_type):
    ...
    try:
        ...
        if (
            container[index - 1] == quote_type
        ):  # and container[index + len(index_to_find)] == quote_type:
...
def receive_answer(
    question: str, possible_answers: list, true_result_answer: str
) -> bool:
    """
    ...
    while True:
        ...
        LOG.error(
            f"Invalid answer, only the following answers are allowed: {','.join(possible_answers)}"
        )
...
def filter_subdict_by_key(dict_with_dict_values: dict, key: str, value) -> list:
    return sort_by_chain_length(
        [
            v
            for k, v in dict_with_dict_values.items()
            if key in v.keys() and v[key] == value
        ]
    )
...
def sort_by_chain_length(dict_with_dict_values: list) -> list:
    return [
        d for d in sorted(dict_with_dict_values, key=lambda x: len(x["chain_tree"]))
    ]
...
def recursive_replace(
    my_content: Any, string_to_find: str, string_to_replace: str
) -> Any:
...
def generate_arg_parser():
    ...
    parser = MyArgumentParser(
        add_help=True, description="TDS client implementation (SSL supported)."
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="whether or not to load existing state ",
    )
    ...
    module.add_argument(
        "-link-name", help="Linked server to launch queries", default=None
    )
    module.add_argument(
        "-max-link-depth",
        help="Maximum links you want to depth recursively",
        default=10,
        type=int,
    )
    module.add_argument(
        "-max-impersonation-depth",
        help="Maximum impersonation you want to depth in each link",
        default=10,
        type=int,
    )
    ...
    module.add_argument(
        "-auto-yes",
        help="Auto answer yes to all questions",
        action="store_true",
        default=False,
    )
    ...
    set_chain = modules.add_parser(
        "set-chain", help="Set chain ID (For interactive-mode only!)"
    )
    ...
    modules.add_parser(
        ...
        help="Retrieve queries to revert to SELF (For interactive-mode only!)",
    )
    get_chain_list = modules.add_parser("get-chain-list", help="Get chain list")
    get_chain_list.add_argument(
        "-filter-hostname",
        help="Get filtered results with specific hostname",
        default=None,
        type=str,
    )
    ...
    set_link_server = modules.add_parser(
        "set-link-server", help="Set link server (For interactive-mode only!)"
    )
    ...
    command_execution.add_argument(
        "-command-execution-method",
        choices=["xp_cmdshell", "sp_oacreate"],
        default="xp_cmdshell",
    )
    command_execution.add_argument("command", help="Command to execute")

    ntlm_relay = modules.add_parser(
        "ntlm-relay", help="Steal NetNTLM hash / Relay attack"
    )
    ntlm_relay.add_argument(
        "smb_server", help="Steal NetNTLM hash / Relay attack (Example: 192.168.1.1)"
    )
    ntlm_relay.add_argument(
        "-relay-method",
        choices=["xp_dirtree", "xp_subdirs", "xp_fileexist"],
        default="xp_dirtree",
    )

    custom_asm = modules.add_parser(
        "custom-asm", help="Execute procedures using custom assembly"
    )
    custom_asm.add_argument(
        "-arch", choices=["x86", "x64", "autodetect"], default="autodetect"
    )
    custom_asm.add_argument(
        "-procedure-name",
        choices=["execute_command", "run_query", "run_query_system_service"],
        default="execute_command",
    )
    custom_asm.add_argument("command", help="Command to execute")

    inject_custom_asm = modules.add_parser(
        "inject-custom-asm", help="Code injection using custom assembly"
    )
    inject_custom_asm.add_argument(
        "file_location", type=str, help="File location to inject"
    )
    ...
    direct_query.add_argument(
        "-query-method", choices=["OpenQuery", "exec_at"], default="OpenQuery"
    )

    retrieve_passwords = modules.add_parser(
        "retrieve-password", help="Retrieve password from ADSI servers"
    )
    retrieve_passwords.add_argument(
        "-listen-port", help="Port to listen on (default 1489)", type=int, default=1389
    )
    retrieve_passwords.add_argument(
        "-adsi-provider",
        help="choose ADSI provider " "(if not defined, it will choose automatically)",
        default=None,
    )
    retrieve_passwords.add_argument(
        "-arch", choices=["x86", "x64", "autodetect"], default="autodetect"
