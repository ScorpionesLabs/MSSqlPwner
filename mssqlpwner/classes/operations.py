...
class Operations(query_builder.QueryBuilder):
    ...
    def connect(self, username: str, password: str, domain: str) -> bool:
        if not utilities.is_port_open(
            self.server_address, int(self.port), self.options.timeout
        ):
    ...
    def add_to_server_state(
        self, chain_id: Union[str, None], key: str, value: Any
    ) -> str:
        """
    ...
    def filter_server_by_link_name(self, link_name: str) -> list:
        ...
        """
        discovered_links = utilities.filter_subdict_by_key(
            self.state["servers_info"], "link_name", link_name
        )
        ...
        servers = utilities.filter_subdict_by_key(
            self.state["servers_info"], "hostname", link_information["hostname"]
        )
    ...
    def generate_authentication_details(self, chain_id: str) -> str:
        ...
        server_user = (
            original_server_user if original_server_user else server_info["server_user"]
        )
    ...
    def generate_chain_str(
        self, chain_id: str, print_authentication_details: bool = True
    ) -> str:
        """
        ...
        for link_name, new_chain_id in server_info["chain_tree"]:
            if print_authentication_details:
                authentication_details = self.generate_authentication_details(
                    new_chain_id
                )
    ...
    def is_same_privileges(self, chain_id: str) -> bool:
        ...
        for server_info in self.filter_server_by_link_name(
            current_server_info["link_name"]
        ):
            if server_info["chain_id"] == chain_id:
                continue
            if (
                server_info["server_user"] == current_server_user
                and server_info["db_user"] == current_db_user
            ):
                return True
        return False

    def detect_architecture(
        self, chain_id: str, arch: Literal["autodetect", "x64", "x86"]
    ) -> Union[str, None]:
        """
    ...
    def is_privileged_user(self, chain_id: str, user_type: str) -> bool:
        ...
        current_user = (
            ...
            if user_type == "server"
            else server_info["db_user"]
        )
        high_privileged_roles = (
            self.high_privileged_server_roles
            if user_type == "server"
            else self.high_privileged_database_roles
        )

        user_roles = (
            server_info["server_roles"]
            if user_type == "server"
            else server_info["database_roles"]
        )
    ...
    def retrieve_server_information(
        self, chain_id: Union[str, None], link_name: Union[str, None]
    ) -> list:
        """
        ...
        enumeration_results = {
            ...
            "server_principals": self.get_impersonation_list(
                chain_id, user_type="server"
            ),
            "database_principals": self.get_impersonation_list(
                chain_id, user_type="database"
            ),
            "available_databases": self.get_database_list(chain_id),
        }
        chain_str = self.server_address
        if chain_id:
            chain_str = self.generate_chain_str(
                chain_id, print_authentication_details=False
            )
        ...
        hostname = utilities.remove_instance_name(
            enumeration_results["server_information"]["results"][0]["hostname"]
        )
        if not link_name:
        ...
        for k, v in {
            ...
            "version": enumeration_results["server_information"]["results"][0][
                "version"
            ],
            "db_name": enumeration_results["server_information"]["results"][0][
                "db_name"
            ],
            "domain_name": enumeration_results["server_information"]["results"][0][
                "domain_name"
            ],
            "instance_name": enumeration_results["server_information"]["results"][0][
                "instance_name"
            ],
        }.items():
        ...
        chain_str = self.generate_chain_str(chain_id)
        self.add_to_server_state(
            chain_id, "chain_str", self.generate_chain_str(chain_id)
        )
            ...
            for principal_results_dict in enumeration_results[key]["results"]:
                for _, principal_results in principal_results_dict.items():
                    ...
                    if principal_results in server_info[key]:
                        continue
                    LOG.info(
                        f"Discovered {principal_type} principal: {principal_results} on {chain_str}"
                    )
            ...
            if self.is_privileged_user(chain_id, principal_type):
                ...
                for principal_user_dict in privileged_users["results"]:
                    for _, principal_user in principal_user_dict.items():
                        ...
                        LOG.info(
                            f"Discovered {principal_type} principal: {principal_user} on {chain_str}"
                        )
        ...
        for key in ["server_principals", "database_principals"]:
            ...
            for principal in server_info[key]:
                ...
                self.add_to_server_state(
                    clone_chain_id, "original_server_user", server_user
                )
    ...
    def retrieve_links(self, chain_id: str) -> None:
        ...
        if len(server_info["chain_tree"]) > self.max_link_depth:
            LOG.info(
                f"Reached max depth for chain {chain_str} (Max depth: {self.max_link_depth})"
            )
            return
        ...
        for row in linkable_servers_results["results"]:
            ...
            for collected_chain_id in self.retrieve_server_information(
                new_chain_id, link_name
            ):
    ...
    def direct_query(
        ...
        chain_id: str,
        query: str,
        method: Literal["OpenQuery", "exec_at"] = "OpenQuery",
        ...
        """
        results = self.build_chain(
            chain_id, query, method, decode_results, print_results
        )
        if not results["is_success"]:
            LOG.error(
                f"Failed to execute query: {query} on {self.generate_chain_str(chain_id)}"
            )
            return False

        if not results["results"]:
            LOG.info(
                f"No results found for query: {query} on {self.generate_chain_str(chain_id)}"
            )
            return True
    ...
    def execute_procedure(self, chain_id: str, procedure: str, command: str) -> bool:
        ...
        self.reconfigure_procedure(
            chain_id, "show advanced options", required_status=True
        )
        self.reconfigure_procedure(chain_id, procedure, required_status=True)

        procedure_results = self.execute_operation(
            chain_id, "procedure", procedure, command
        )
        chain_str = self.generate_chain_str(chain_id)
    ...
    def add_new_custom_asm(
        self, chain_id: str, asm_file_location: str, asm_name: str
    ) -> bool:
        """
        ...
        if not self.reconfigure_procedure(
            chain_id, "show advanced options", required_status=True
        ):
    ...
    def execute_custom_assembly(
        ...
        **kwargs,
        ...
        if not self.create_operation(
            chain_id, operation_type, asm_name, operation_name, args, db_user, **kwargs
        ):
            LOG.error(f"Failed to create custom {operation_name}")
            return {"is_success": False}

        execution_results = self.execute_operation(
            chain_id, operation_type, operation_name, command, wait, db_user
        )
    ...
    def add_rev2self_query(self, chain_id: str, query: str, template: str) -> None:
        ...
        self.rev2self[chain_id].append(
            utilities.replace_strings(template, {"[PAYLOAD]": query})
        )
